...
const bookHouse = (bookedCollection, app) => {
    app.post('/bookHouse', async (req, res) => {
        try {
            const bookedhouse = req.body;
            const result = await bookedCollection.insertOne(bookedhouse);
            res.send(result);

        }

        catch (err) {
            res.status(500).send('Error inserting bookedhouse into the database');
        }
    });
};

module.exports = { RegisterUser, LoginUser, userData, addHouse,bookHouse };
